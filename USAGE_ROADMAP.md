...
> U17 当前设计（2026-09-13）：[产品设计](docs/U17_DESIGN.md)与[开发规划](docs/U17_DEVELOPMENT_PLAN.md)。方向已确认，已进入 M0 验证与基础能力开发；新想法与已有项目改进均纳入首版，集成现有 agent 引擎并自建项目编排。下文相关旧规则已修订，历史工期估算不作为本次承诺。
...
- [ ] M0（进行中）：完整类型资料读取、OpenCode 异步执行/权限恢复/状态投影、工作区文件基线、含来源/需求/决定的独立项目存储及不依赖旧会话的交接包已实现；继续完成双场景样例文档包和迭代对象。过程见 [验证记录](docs/U17_M0_VALIDATION.md)。
- [ ] M1：多项目、类型与应用搜索阅读、项目资料集与来源版本。
- [ ] M2：分析取舍、关联文档、人工修改、范围检查、版本与独立导出。
- [ ] M3：已有项目基线、变更规划、外部修改核对与后续迭代。
- [ ] M4：一个 agent 引擎的集成、执行范围、恢复与成果回收；模拟旧会话不可访问并验证任务交接。
- [ ] M5：简单新想法、已有项目改进、多类型需求三个试点，至少一次后续修改；记录对照与真实问题。
...
- [ ] F2 归类助手原型（检索 + 边界解释）
- [ ] U17 项目资料、范围取舍、关联文档与 F6 一致性检查（按 M1–M3）
- [ ] 里程碑 2：F7 后端（pglite + agent 自动调研闭环，U14 落地）
- [ ] 里程碑 3：U18 实例拆解批量生产，充进 teardowns
- [ ] F8 集成现有 agent 引擎并回收成果（按 U17 M4–M5，属于首版，不依赖 U18 批量生产）
