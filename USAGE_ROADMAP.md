...
> U17 当前设计（2026-09-13）：[产品设计](docs/U17_DESIGN.md)与[开发规划](docs/U17_DEVELOPMENT_PLAN.md)。方向已确认，M0 核心验证完成并进入 M1；新想法与已有项目改进均纳入首版，集成现有 agent 引擎并自建项目编排。下文相关旧规则已修订，历史工期估算不作为本次承诺。
...
- [ ] M1（进行中）：M1-01 项目创建、保存与切换已实现；继续统一搜索、完整资料阅读与项目资料收藏。
