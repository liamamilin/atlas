...
> 状态：调研完成，待完工后逐项启动。本文档记录"1805 篇文档买回来之后怎么用"。
> 日期：2026-09-07。生产进度：~711/1805 叶子完成（4 worker 运行中）。
...
**U17 — Atlas 驱动开发（深度设计版，2026-09-07）**

- 定位：把 atlas 从"知识资产"变成"开发流水线的上游"，对内杠杆最高的用法。核心洞察：**叶子回答"这类应用本来该是什么"，方案只回答"这次具体做什么"——方案是叶子与想法的差集，不是叶子复述**
...
```
想法 → ① 检索叶子(U2)
     → ② 显式验证：agent 对照候选叶子的 L0 + 边界问题自问
        "这个想法真的属于这个叶子吗？"（不匹配 → 打回重检索，防方案继承错误结构）
     → ③ 第 0 步：要不要建（对照叶子 Representative Products，成熟产品已覆盖 → 转选型 U10）
     → ④ 生成「方案」（一次 LLM 会话，~$0.2，一页以内）
     → ⑤ 【人工闸门】评审方案 → 放行
     → ⑥ agent 开发（注入：开发卡 + 方案）
     → ⑦ 验收（U12 检查表 + 排除项核查 + 自检对照 L1）
```

**模式 B：差距审计**（可能更高频：多数开发是改旧系统）
```
已有系统 → U3 分类到叶子 → 对照该叶子 L1 成熟结构清单
→ 输出差距报告：缺什么（L1 有而系统没有）、多了什么（超出类型的镀金）
→ 差距直接变 backlog → 走模式 A 的 ④-⑦
```
atlas 在此扮演**完整性预言机**——"这类产品成熟形态是什么、你缺哪块"，市面上没有等价物。

**模式 C：组合构建**（复合想法）
```
"带考勤的排班系统" → 拆分：主叶子（骨架 + 数据主权）+ 副叶子（模块）
→ 组合方案必须定义拼接边界：共享实体、数据归属、交接点
```

#### 方案模板（只记差异）

1. 定位：主叶子 slug +（副叶子）+ 一句话想法
2. 用户与场景：从叶子 Users & Context 中**选定**实际服务谁
3. 范围三层（防爆炸的关键）：Core（L0 必做）/ Standard（L1 选用，**MVP ≤3 项**）/ Excluded（明确不建 + 理由，含"建了会漂向 X 类型"的禁令）
4. 拼接边界（模式 C 必填）：共享实体表、数据主权、交接点
5. 接口：从叶子 Interfaces 中选定
6. 技术栈与部署：想法自有信息，叶子不提供
7. 关键规则：叶子 Important Rules 子集 + 自定义
8. 验收：U12 检查表引用 + 自定义验收

#### 关键工件：开发卡（dev card，~50 行）

编码 agent 上下文装不下 280 行叶子文档 → 从 atlas.json 派生开发卡：
`L0 不变式 + 核心结构树 + 选定检查清单 + 边界禁令（"不要做 X，否则产品变成 Y 类型"）`
→ **U17 的可行性直接依赖 U1 导出**，开发卡是 atlas.json 的固定下游产物（可在 U1 导出脚本中一并生成）。

#### 三个失败模式与对策

| 失败模式 | 对策 |
|---|---|
| 叶子匹配错 → 方案继承错误结构 | 流水线第 ② 步显式验证，不匹配打回 |
| 范围爆炸（L1 全抄 → 小瀑布） | 模板强制选择 + MVP ≤3 项硬约束 |
| 开发走偏（偷加方案外功能） | 验收双向核查：该做的做了 + 不该做的没做（对照排除清单） |

#### 质量指标（试点实测）

- 匹配准确率：验证步打回率（理想 <20%）
- 走偏率：验收时发现超出方案范围的功能占比
- 完整性：交付物覆盖选定 L1 项的比例
- 方案评审成本：人工修改方案的字数比例（衡量"只记差异"原则执行度）
...
- U3 分类 → 模式 B 入口
- U12 检查表 → 模式 A ⑦
- 方法论复用：atlas 生产流程（leaf → research → doc）本身就是一套 agentic 流水线，U17 的 agent 设计可参考 `atlas-writer` 的经验（steps、工具纪律、自检模式）
...
| 方案 | PlanItem（v1/later/skip + 理由 + 来源保留） | U17 方案（只记差异） |

#### 直接采纳（3 项）

1. **U17 方案吸收 PlanItem 结构**：`decision: v1需要/以后做/不采用` + 采用理由 + 来源保留，导出按取舍状态分组——这是"只记差异"原则的已验证交互形态
...
**第二层：atlas 独有差异化（MVP 结构性做不到）**
- F4 建还是买决策器：需求 → 叶子 → 成熟产品清单+深拆 → "买/配置/自建"建议（MVP 停在了解竞品，这步走到决策）
- F5 空白雷达：类型 × 产品覆盖稀疏热力图 → 产品机会地图 / 创业方向库（1805 类型，无人做过）
- **F6 方案边界 Lint**（首推）：用户在方案中勾选功能时，用叶子边界判据实时校验——"选了 prompt 生成功能，产品会漂向 AI 图像生成类，确认吗？"。方案自带纪律，竞品无法复制
- F7 提交 → 自动调研闭环（战略级）：用户提交缺失的类型/软件 → agent 自动调研 → 生成新叶子/新拆解 → 进库（= U14 自进化，静态库变活生态）

**第三层：生态级**
- F8 方案 → 开发流水线一键对接：方案导出 → coding agent（带开发卡，U17 闭环）
- F9 产品变化监控：verifiedAt 雏形 → 定期重查触发重新拆解
- F10 协作分享：方案/对比可分享可协作（MVP 已预留 better-auth）

**优先级**：F1+F2 先做（一个周末原型，验证类型轴体验）→ F6（便宜+独有+强化 U17）→ F7（活生态关键）→ 其余按需。
...
### 阶段 4：U17 Atlas 驱动开发试点（U2 就绪后）
- [ ] U1 导出脚本中一并生成「开发卡」（每叶子 ~50 行：L0 + 结构树 + 检查清单 + 边界禁令）
- [ ] 选定一个模式 B 试点（差距审计：真实旧系统 → 分类 → L1 差距报告 → backlog），这个模式风险低、见效快
- [ ] 再做一个模式 A 试点（真实小需求走完整流水线：验证步 → 方案 → 人工评审 → agent 开发 → 双向验收）
- [ ] 实测记录四项指标：验证步打回率、走偏率、L1 覆盖完整率、方案评审修改比例
- [ ] 组合型想法（模式 C）验证多叶子拼接方案格式
- [ ] 试点达标后固化为标准开发流程，atlas 由此成为团队开发基础设施
