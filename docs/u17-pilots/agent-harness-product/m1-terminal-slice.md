...
状态：已实现并完成当前范围验收；真实 CLI、获准只读工具调用、固定验证、证据保存、清理和完整只读 TUI 路径均已通过；单文件变更任务的执行、显式应用、独立验收和清理也已通过。停止、任务级超时和 OpenCode 失败恢复链路已通过；应用后回滚仍需另建任务验收。
...
- `atlas_cli.py --interactive --lang zh` 已复核项目选择、任务启动确认、运行详情、产品验收、导出、清理和退出（退出码 0）；标准 PTY 不支持 `curses.curs_set(0)` 时已安全降级，并修复任务视图 `task_index` 闭包错误。变更任务的停止、任务级超时和 OpenCode 失败恢复链路已通过；应用后回滚路径另列为后续验收范围。
