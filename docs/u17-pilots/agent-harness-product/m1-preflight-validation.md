...
状态：Atlas 侧接入与回归验证已完成；真实终端纵向切片已在授权环境完成 CLI、隔离副本、固定验证、权限门、获准只读工具调用、证据和清理复测。只读任务的完整 TUI 路径已通过；另一个受控单文件变更任务已完成执行、固定验证、显式应用、独立验收和清理。停止、取消/超时、失败恢复和回滚仍需单独验收。
...
结论：Atlas 的 CLI 启动、隔离、权限门、获准只读工具调用、固定验证、证据保存和清理链路已经有真实运行证据；该只读验收任务在后续 TUI 路径复核后已通过，变更任务的停止、取消/超时、失败恢复和回滚仍未覆盖。
...
该结果收口终端入口的项目选择、任务查看和返回稳定性；只读任务的启动、执行、验收、导出和清理路径已在后续复测中通过。单文件变更任务的执行、应用、独立验收和清理已另行通过；停止链路已通过；取消/超时、失败恢复和回滚仍需另建任务验收。
...
结论：一次明确获准的只读工具调用已通过；随后只读任务的 TUI 执行、查看、验收、导出和清理路径也已通过。该结果不能替代变更任务的停止、取消/超时、失败恢复和回滚验收；单文件变更应用验收已有独立记录。
...
本次接入完成 Atlas 侧的 preflight 门、可视反馈、显式 CLI 执行、CLI 运行摘要归一化、固定验证步骤映射、独立证据目录写入、execution record 索引、显式清理入口、`evidence.run_summary` 统一字段和最小终端入口；只读任务的 TUI 执行、查看、验收、导出和清理路径已通过，但不代表 OpenCode TUI 已经由 Atlas 重实现。停止链路已通过；尚未覆盖的是取消/超时、失败恢复和回滚按钮，仍需继续创建针对性的 `code` 或 `document` 任务验收；本轮单文件变更的执行、应用、独立验收和清理证据见 [`m1-mutation-acceptance.md`](m1-mutation-acceptance.md)。若 provider 权限或本机配置仍阻塞，则把该访问条件记录为真实部署前置条件。
