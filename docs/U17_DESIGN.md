...
日期：2026-09-13  
状态：产品方向已确认；本文细化交互与工程边界，尚未实现。  
配套：[开发规划](U17_DEVELOPMENT_PLAN.md) · [路线图](../USAGE_ROADMAP.md)
...
| 能力 | 当前基础 | U17 需要补齐 |
|---|---|---|
| 类型与应用知识 | 1,805 个正式类型及对应研究笔记，33 个应用实例 | 项目内收集资料、摘录、引用和版本记录 |
| 搜索 | 网页为类型关键词搜索；MCP 有类型向量搜索及分类 | 在同一入口搜索类型与应用，进入全文阅读；分类仅作建议 |
| 阅读 | 类型详情提供正文、关系、来源及英文原文 | 将研究笔记与可用应用资料纳入阅读；区分简介与深度分析 |
| Agent | 草稿生成通过 OpenCode CLI 执行 | 可恢复的执行记录、工作区管理、事件、结果和文件差异关联 |
| 项目与文档 | 尚无项目资料集、文档版本或迭代模型 | 新增项目持久化及关联文档工作流 |
...
创建项目只需名称和简短想法，已有项目可同时关联本地目录。允许先在全站搜索和阅读，再把资料加入新项目，不强制先完成需求表单。

首版支持多个项目；同一时间的执行并发保持简单。用户切换项目时，资料、文档和任务不能混用。
...
| 编号 | 场景 | 可观察的完成条件 |
|---|---|---|
| U17-A1 | 从搜索开始的新想法 | 找到类型与应用，阅读后收集资料；生成方案中的关键引用可定位原文 |
...
这些是开发前的验证项，不改变已确认的产品方向：

- OpenCode 在当前本地环境能否提供可靠的会话恢复、进度和文件差异；需要补多少编排能力。
- 无法访问旧会话时，哪些任务上下文仍未进入 Atlas 持久化记录；用 §9.4 的交接验证发现并补齐。
- 六类默认文档怎样合并才能降低阅读和维护负担；以真实案例决定默认呈现方式。
- 人工编辑与关联需求同步的交互是否清楚，如何减少错误的关联更新。
- 当前实例资料能支持多深的产品分析，哪些案例需要定向补充资料。
- 无 Git 项目的执行工作副本与差异回收方式；在此明确前可先提供读取、分析和导出。

开发顺序、验证任务与依赖见[开发规划](U17_DEVELOPMENT_PLAN.md)。
