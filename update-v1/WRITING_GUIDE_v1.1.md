...
# 28. Canonical Core Must Be Minimal
...
Optional / Variant Structure
Vendor-specific Detail
```

Do not make the definition carry everything that modern products commonly support.

## Example

Too strong:

> An Instant Messaging Application is a phone-number-based application with an address-book contact list, delivery receipts, voice/video calling and multi-device sync.

Better:

> An Instant Messaging Application enables addressable private or small-group message exchange between identifiable participants with persistent conversation history.
...
For every item in the Core Model, ask:

```text
Is this required to recognize the Type,
or merely common in today's market?
```

If it is merely common, move it out of the defining core.

---

# 29. Evidence-Calibrated Writing

Final prose must reflect how strongly a finding is supported.

## Strong wording

Use only when the evidence and abstraction are strong:
...
Use for cross-product commonality:
...
## Weak / qualified wording

Use when evidence is limited or product-dependent:

```text
Some products...
Depending on the product...
A common variant...
The researched sample suggests...
```
...
unless those details are directly researched and materially useful.

Precise vendor facts usually belong in Research Notes, not the Canonical Application Document.

---

# 30. Separate Concept from Implementation

Whenever possible, write the conceptual layer first and the implementation layer second.
...
```text
Concept:
Relationship / Reachability Graph
...
This prevents the document from confusing:
...
```text
how specific products implement it
```

---

# 31. Research Notes May Be Richer Than the Final Document

Do not force every researched detail into the Application Document.

A useful three-layer filter is:

```text
Must understand
→ Core Model / How It Works / Boundary

Helpful to understand
→ Main Interfaces / Important States / Rules

Product detail
→ Research Notes
```

Examples of details that often belong only in Research Notes:

- exact edit windows
- exact group-size limits
- detailed device-pairing behavior
- specific encryption modes
- vendor-specific blocking behavior
- plan-specific capabilities
- branded module names

The final Application Document should optimize for understanding, not evidence exhaustiveness.

---

# 32. Final Canonicalization Check Before Publication

Before finalizing a document, perform this additional check:

```text
[ ] Is the definition smaller than the full modern feature set?
[ ] Did I separate conceptual structure from common implementation?
[ ] Did I accidentally define the Type using one era, region, or vendor pattern?
[ ] Does every precise claim have matching evidence?
[ ] Are single-product findings still marked product-specific/optional?
[ ] Could the document be shortened without losing the Application's mental model?
```
...
# 33. Final Document Must Not Expose Internal Research Framework

Research Notes 是内部工作文件，可以自由使用方法论术语：

```text
L0 / L1 / L2 / L3
Evidence Layer (A / B / C)
Canonical Inference
```

Final Application Document 是面向读者的成品，必须**自然地**解释 Application：

```text
Overview → Core Model → How It Works → Interfaces → Rules → Variants → Related Types → Products → Sources
```

## 最终文档中不得出现

- L0 / L1 / L2 / L3 标签
- "The Application Document presents only..."
- "Concept → Implementation Separation"（这类内容可以保留，但标签必须去掉，用自然标题替代）
- `WORKFLOW_v1.1.md §xx` 或 `WRITING_GUIDE_v1.1.md §xx` 引用

## 自然语言替代

```text
L0 Defining Invariant        → the defining core
L1 Common Mature Structure   → standard capabilities / what mature products add
L2 Variant / Optional        → common variants / optional capabilities
L3 Vendor-specific           → （不出现，留在 Research Notes）
```

## 唯一例外

Sources 中可以说明研究局限（来源不可达、证据降级、断言强度校准），但同样不得引用 § 编号。

## 原则

> 研究框架服务于研究过程。读者看到的应该是解释，不是方法论的脚手架。
