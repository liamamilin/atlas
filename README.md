...
截至 2026-09-13，U17 的 M0–M4 已实现。网页除搜索、资料、需求、决定、关联文档、项目基线和改进方案外，现在可建立迭代和范围化任务，让 OpenCode 在隔离工作副本中执行，处理权限、提问、停止和不可用状态，并在同一会话与副本中继续修改。Atlas 为每轮续作建立独立快照，核对自有文件快照和结构化验证命令，校验 agent 按需求 ID 报告的覆盖、未完成项和方案偏离，再以冲突检查和失败回滚把审阅结果写回源项目。被续作或重试取代的结果不能应用；精确写回状态成为新项目基线，产品验收仍与 agent 完成分开记录。schema 5 项目导出包含迭代、执行/应用证据和不依赖旧会话的交接材料。M2 批量生成/自然语言修改和 M5 完整试点仍未完成。
