...
截至 2026-09-13，U17 的 M0–M4 已实现。网页可选择核心四件套、完整六件套或自定义文档集合；Atlas 要求目标完整、统一排序，并在逐份审阅保存时把下游草案绑定到本轮上游的不可变版本。文档可选择唯一 Markdown 标题，用自然语言生成严格限于该章节的修改候选，查看 diff 后保存为新版本。可选价值分析独立整理事实、未知、推断、风险与验证建议，不评分、不确认需求、不改变范围。开发执行继续由 OpenCode 在隔离工作副本中完成，Atlas 独立记录快照、文件/命令证据、需求覆盖、冲突安全写回和产品验收。下一步按 M5 计划完成普通生成对照、三个真实案例和至少一次后续修改。
