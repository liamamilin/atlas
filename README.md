...
截至 2026-09-13，U17 的 M0–M3 与 M4 核心执行闭环已实现。网页除搜索、资料、需求、决定、关联文档、项目基线和改进方案外，现在可建立迭代和范围化任务，让 OpenCode 在隔离工作副本中执行，处理权限/提问/停止状态，核对 Atlas 自有文件快照和结构化验证命令，并校验 agent 按需求 ID 报告的覆盖、未完成项和方案偏离，再以冲突检查和失败回滚把审阅结果写回源项目。精确写回状态成为新项目基线，产品验收仍与 agent 完成分开记录。schema 5 项目导出包含迭代、执行/应用证据和不依赖旧会话的交接材料。M4 的会话内续作和完整失败收口仍在实施；M2 批量生成/自然语言修改仍未完成。
