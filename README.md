...
截至 2026-09-13，U17 的 M0 核心验证、M1 项目资料集、M2 固定输入分析和关联文档生成闭环，以及 M3 工作区基线已完成：资料全文读取、OpenCode 可替换执行器、执行状态投影、独立项目/迭代存储、双场景文档样例和离线任务交接包已落地；网页支持项目创建与切换、统一搜索、正式资料阅读与收藏、需求推荐/确认、决定、固定输入分析、逐条采用候选、按类型生成文档草案、不可变版本、上游变化复核、独立 Markdown ZIP 导出，以及工作区文件/Git 指纹、证据分层、变化范围和审阅后采用。M2 的批量生成/自然语言修改与 M3 的目标驱动改进方案仍在开发。当前应用的运行功能以本 README 其余章节为准。

U17 的本地 API 提供 `GET /api/sources/<slug>` 查询正式类型资料清单，通过 `GET /api/sources/<kind>/<slug>` 按完整文档或章节分页读取；项目 API 管理项目、固定资料、需求、用户确认、决定、关联文档版本、生成运行、工作区基线、差异与导出。网页的“项目”入口可进入完整项目工作区。MCP 同步提供 `get_atlas_source_manifest` 与 `read_atlas_source`。`project_generation.py` 负责固定输入、OpenCode 隔离调用、结果校验和逐条采用；`workspace_snapshot.py` 提供文件指纹，`project_baseline.py` 负责读取覆盖范围、Git 状态、证据分层及审阅后采用。目标驱动的已有项目改进方案和正式开发执行流程尚未实现。
