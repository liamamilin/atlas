...
截至 2026-09-13，U17 的 M0–M4 已实现，M2 的依赖化文档集合和章节级自然语言修改也已补齐。网页可选择核心四件套、完整六件套或自定义文档集合；Atlas 要求目标完整、统一排序，并在逐份审阅保存时把下游草案绑定到本轮上游的不可变版本。文档可选择唯一 Markdown 章节，用自然语言生成严格限于该章节的修改候选，查看 diff 后保存为新版本；章节外人工内容和并发新版本受到校验保护。开发执行继续由 OpenCode 在隔离工作副本中完成，Atlas 独立记录每轮快照、文件/命令证据、需求覆盖、冲突安全写回和产品验收。M2 只剩可选价值分析，M5 完整试点仍未完成。
