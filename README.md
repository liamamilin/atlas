...
截至 2026-09-13，U17 的 M0–M4 已实现，M2-02 的依赖化文档集合也已补齐。网页可选择核心四件套、完整六件套或自定义文档集合；Atlas 要求目标完整、统一排序，并在逐份审阅保存时把下游草案绑定到本轮上游的不可变版本。页面同时提供生成历史、简要/完整草案视图和项目路径提示。开发执行继续由 OpenCode 在隔离工作副本中完成，Atlas 独立记录每轮快照、文件/命令证据、需求覆盖、冲突安全写回和产品验收。schema 5 项目导出包含迭代、执行/应用证据和不依赖旧会话的交接材料。M2 自然语言修改、价值分析和 M5 完整试点仍未完成。
