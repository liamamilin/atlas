# Scientific Core Facility Management

## Overview

A **Scientific Core Facility Management** application is the operating system of a shared scientific core facility — a "core facility" or "shared resource" that offers expensive scientific instruments and specialized services to a population of users drawn from many groups, and recovers its operating costs through usage-based charging.

It solves a specific operational problem: an institution buys instruments (sequencers, microscopes, flow cytometers, mass spectrometers, NMR spectrometers) that no single lab can afford or fully utilize, staffs them with experts, and opens them to a user community. The facility must control who may use what, schedule time on instruments, accept and fulfill service requests, record who used what for how long, and turn that recorded usage into charges billed to the groups' grants, departmental or hospital accounts, or external organizations.

The defining structure is small:

```text
Shared instrument/service catalog (rate-bearing, presented as the facility's storefront)
└── Identified user (member of a group with funding sources)
...
A note on naming: the market uses **"scientific core facility"**, **"research core facility"**, and simply **"core facility"** for the same kind of operation, and vendors sell one product category — core facility management software — to universities, research institutes, hospitals and medical schools alike. This document is written from the scientific-facility lens, with emphasis on the healthcare and cross-disciplinary breadth of the customer base; the atlas documents the same Type from the research-facility lens under **Research Core Facility Management**. The alias is recorded in the atlas status for taxonomy review, and each document points at the other.

Its boundary: take away the usage-to-recharge economics and it becomes a resource calendar; take away the shared multi-group facility context and it becomes single-lab scheduling tooling; take away scheduling and charging and keep sample-and-assay workflows and it becomes a LIMS; replace the instruments and services with patient specimens and clinical orders and it becomes clinical laboratory territory (LIS), which is a different system even inside the same hospital.
...
- **Researcher / core customer** — a group member who reserves instrument time, submits service requests, selects the payment source, and tracks their own requests and reservations.
- **Principal investigator / group leader** — the financial authority over a group: assigns payment sources to group members, approves spending above thresholds, and sees what the group is being charged.
...
Typical context: universities and research institutes are the center of gravity, but the same software runs core facilities in **hospitals and academic medical centers**, medical schools, pharmaceutical and biotech companies, contract research organizations, and shared/incubator laboratories — and, outside the life sciences, in engineering schools and analytical testing or manufacturing labs. Facility types include genomics, sequencing, proteomics, flow cytometry, light and electron microscopy, histology, mass spectrometry, metabolomics, NMR, structural biology, vector production, and bioinformatics cores, plus hybrid and emerging facility types. A single institution commonly operates many such facilities on one deployment of the system — including deployments that span a university and its medical school.
...
- **Reservation** — a claim on a specific time slot, made by an identified user under a specific group and payment source.
- **Service and service request** — work the facility performs for a user (e.g., run samples, prepare libraries, image specimens, produce vectors). A request carries forms/questionnaires, a quote where cost is not fixed, an approval, staff assignment, and progress phases.
- **User, group, group leader** — the user population is organized into groups led by a financially responsible leader; the group is the financial responsibility unit.
- **Fund / payment source** — the vehicle a charge is billed to. The same concept appears under many institutional names: grant number, chart-field string, speed code, account number, project ID, hospital cost center — or, for external customers, a purchase order.
...
Implementations:    grant number, chart-field string, speed code, account number,
                    project ID, hospital cost center, purchase order, credit card
...
A reader who has only seen one implementation (e.g., a university genomics core billing grant numbers monthly) should still be able to recognize the others (a hospital's imaging core billing departmental accounts; a commercial shared lab invoicing purchase orders; a subsidized core where charges resolve to zero) from the core model.

## How It Works

### Onboarding: from user to qualified customer

```text
User registers (often with institutional credentials via SSO)
→ joins a group (or requests membership)
→ group leader or manager assigns payment sources to the group's members
→ user completes training for the instruments they need
...
→ select a time slot
→ choose group + payment source
→ complete required forms (if any)
→ submit (auto-confirmed, or pending facility approval)
...
→ financial approval by the group leader / manager (auto-approved below a threshold)
...
→ group leaders see what their groups consumed; finance reconciles
```
...
- identified-user reservation/request with group and payment-source attribution
...
**Optional / variant** — depends on segment, funding model, and discipline:
...
- sample manifests and sample-tracking add-ons
- consumables/inventory management
- animal facility management, equipment loans, facility floor plans (module-level adjacencies in some products)
...
- primary actions: create a reservation (select a slot), move/resize it, cancel it, view pricing and rules

### Reservation form

The transaction surface of the self-service loop.

- group selection, payment source, usage type, cost estimate, required forms, linked resources, repeating options, notes
...
A reservation or request is not merely "booked" — it carries a payment source, and spending typically requires approval by the group's financial authority (the group leader or a designated manager) before the facility starts work or the reservation is confirmed. Amounts below a configurable threshold are commonly auto-approved. This makes the group structure an access-control structure, not just an org chart.

### Reservations have states and economics

A reservation may be auto-confirmed or pending facility approval; approved reservations are commonly locked against casual change. Cancellation is rule-governed: deleting far enough ahead of the slot is free, late cancellation becomes a cancellation (often with a fee that must be accepted), and no-shows may be charged. These rules protect instrument utilization — the facility's scarcest resource.
...
Funds expire (grants end, budget periods close); the system warns users when a chosen fund is near expiry. External customers may need formally approved standing purchase orders before they can pay at all.
...
- **Instrument-based (self-service) cores** — the user operates the instrument; the system's center is scheduling, access, and time-based charging.
...
- **Single facility vs institution-wide deployments** — one core adopting a tool vs an institution standardizing many cores on one system with cross-facility oversight and consolidated reporting — including deployments spanning a university and its medical school.
- **Healthcare-hosted cores** — cores inside hospitals and academic medical centers run the same model; their funding sources are commonly departmental or hospital accounts alongside grants. Patient-facing clinical testing is not part of this Type (see Related Types).
...
- **Non-biomedical scientific cores** — engineering, analytical testing, and manufacturing/quality contexts apply the same machinery to their own shared instruments.
...
|---|---|---|
| Research Core Facility Management | **alias — same market family** | vendors sell one core-facility category to universities, hospitals, and companies alike; that document holds the research-facility lens, this one the scientific-facility lens. Alias recorded for taxonomy review |
...
| Laboratory Information System / LIS | adjacent on the healthcare side | LIS runs patient/clinical-specimen testing inside a hospital's clinical lab, with clinical orders and results to patient records; a hospital's research core facility serves researchers with booking/access/recharge machinery. A hospital commonly runs both, on different systems. |
| Scientific Instrument Management | overlapping module | Instrument registry, maintenance, calibration and lifecycle are one operational ring here; the center of this type is the facility's service operation to a user population, not the asset lifecycle itself. |
...
| Healthcare Revenue Cycle / laboratory billing | different money path | Charging for clinical services to patients and payers is healthcare revenue-cycle machinery; the recharge loop here bills internal funding sources and external research customers, not payers. |
| Enterprise Resource Scheduling Platform | generic neighbor | Generic resource scheduling lacks the facility semantics: qualification-gated instrument access, grant- or account-funded payment, internal/external rate tiers. |
...
The alias with **Research Core Facility Management** is the defining relationship for the atlas: the two directory names resolve to one market family, evidenced by the vendors' single product category, the "scientific core" vocabulary used for the same products, and hospital/medical-school customers running the same structure as university cores. The most important substantive boundaries are the two economic seams: against the Resource Calendar (bookings without economics) and against clinical laboratory systems (patient testing inside the same hospital, on different software).

## Representative Products

- Agilent iLab Operations Software — the widely deployed academic and medical-center core facility platform; storefront-and-fund model
- BookitLab (Prog4biz) — modular platform spanning scheduling, access control, requests, assets, and billing; self-describes as built for scientific core operations
- Stratocore PPMS — platform with a price/subsidy-rule and project-centric spine; universities, hospitals, and companies among its stated customers

The defining core was checked against healthcare-hosted facilities (hospital and medical-school cores among the sampled vendors' customers), non-biomedical scientific cores, and against the paper-era practice (sign-up sheets, usage logbooks, recharge invoices) to avoid over-fitting the definition to the current university recharge implementation.
...
- Agilent iLab Help Site — https://help.ilab.agilent.com/ (Key iLab Terms glossary)
- BookitLab — https://www.bookitlab.com/ (home) and https://www.bookitlab.com/solution/core-facility-management ("Core Facility Management Software")
- Stratocore PPMS — https://www.stratocore.com/ (home) and https://www.stratocore.com/solutions/core-facilities ("Core Facility Software")

> Sourcing limitations: the leading product's main marketing page was not retrievable from the research environment; its official help site was used instead, which is the stronger source for operational structure. A fourth candidate product could not be reached and was dropped; no statement in this document depends on it. Institutional "scientific core facility" pages were not reachable, so institution-side naming is evidenced through the vendors' own customer references. Precise vendor-specific figures (customer counts, licence-tier limits, approval thresholds) are intentionally not stated here; they remain in the paired Research Notes.

Detailed evidence, product-by-product observations, the cross-product comparison, the alias determination, and the historical / market-breadth check are recorded in the paired Research Notes.
