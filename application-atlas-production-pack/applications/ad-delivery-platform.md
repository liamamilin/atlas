...
Everything else commonly associated with modern ad serving — real-time bidding, header bidding, unified auctions, first-party audience segments, forecasting, self-service portals, retail-media formats — is widespread in current products but is not part of the defining core. Older and self-hosted ad servers from the tag-and-banner era satisfy the same definition, as do in-house commerce-media engines, without any of those specifics.

When the per-request decision disappears (scheduled sends, planned buys only), or when the paid, accountable nature of the content disappears (organic content selection), the product has drifted into a different Application Type.

## Users & Context

The primary users are advertising operations staff — often called ad ops or traffickers — who translate sold deals into running delivery: they create campaigns, attach creatives, set targeting and goals, and watch delivery against commitments.
...
- **advertisers and agencies** — increasingly through self-service portals: create their own campaigns, monitor delivery and performance
...
The work context is a live production system: campaigns run against real traffic in real time, delivery commitments have financial consequences, and mistakes (wrong targeting, broken creative, mis-set caps) are immediately visible in production placements.
...
Five properties. If any one is removed, the product is no longer recognizable as an ad delivery platform:
...
- A **placement** (commonly called a zone) is a defined ad position on a surface, with an expected size or format, exposed to the platform through a tag, SDK, or API endpoint. Groupings of placements (sites, channels, placement groups) allow campaigns to be aimed at many positions at once.
- The **creative** is the actual displayed content — image, video, native composition, or a reference to a third-party-hosted ad — held in a library and attached to ads.
...
A typical modern product carries most of the following. They are not what makes the product an ad delivery platform, but they make it operable at scale:

- **Targeting** — conditions an ad places on its own delivery: geography, device and platform, keyword or contextual signals, custom key–values passed with the request, audience segments (first-party or third-party), day and time windows, and frequency capping (limit exposures per user over a period).
...
- **Creative management** — uploading and hosting creatives, size and type validation, third-party ad tags, macros for click and data passing, and rotation strategies across multiple creatives of one ad (even, weighted, or auto-optimized by performance).
...
```text
Monitor delivery vs goals (pacing dashboards, line-item/campaign views)
→ adjust: raise bids/weights, loosen targeting, fix under-delivery; cap or pause over-delivery
...
**Defining core** — without these, not an ad delivery platform:
...
**Variant / optional** — depends on side, surface, scale, and business model:
...
Platforms enforce rules about which creatives may appear on which inventory (content categories, brand restrictions, competitive exclusions), typically with stricter rules winning — for example, network-level restrictions that an individual publisher cannot loosen.

### Campaign states

Campaigns and their mid-layer units move through states — inactive (being prepared), active (delivering), completed/expired (goal reached or dates passed), archived (hidden from working lists). State changes are the operator's main lever on live traffic.
...
|---|---|---|
| Ad Server | probable alias | in market usage the two names describe the same functional system — the request-time selection and delivery engine; "ad delivery platform" is the delivery-centric framing. Flagged for a joint review of the two directory entries |
...
| Advertising Campaign Management | upstream | plans and manages campaigns (budgets, channels, trafficking instructions) but does not execute request-time delivery; its output is trafficked into a delivery platform |
...
The boundary with DSP and SSP is the most important one, because enterprise products bundle all three. The structural test is the unit of work: the delivery platform's unit is the managed ad pool and the per-request selection on known placements; the DSP's unit is the bid response in external auctions; the SSP's unit is the packaged inventory offering to buyers.

## Representative Products

- Kevel — API-first ad server; retail media and sponsored-listing infrastructure
- Microsoft Monetize (Xandr) — enterprise publisher suite combining ad server and selling/buying machinery
...
- Kevel — official developer documentation: Introduction to Kevel, Ad Decision Engine Overview, Priorities / Waterfall, Flights — https://dev.kevel.com/docs/understanding-kevel.md , https://dev.kevel.com/docs/delivery-basics.md , https://dev.kevel.com/docs/priorities.md , https://dev.kevel.com/docs/flights.md
- Microsoft Monetize (Xandr) — official product documentation on Microsoft Learn: About Monetize, Object Hierarchy, Guaranteed Delivery Line Item — https://learn.microsoft.com/en-us/xandr/monetize/about-monetize , https://learn.microsoft.com/en-us/xandr/monetize/object-hierarchy , https://learn.microsoft.com/en-us/xandr/monetize/create-a-guaranteed-delivery-line-item-gdali
- AdButler — official help center: What is AdButler?, How AdButler serves ads, Glossary of Terms — https://www.adbutler.com/help/article/quick-start , https://www.adbutler.com/help/article/how-adbutler-serves-ads , https://www.adbutler.com/help/article/glossary
- Revive Adserver — official repository README (site returned HTTP 403; README used as the official description) — https://github.com/revive-adserver/revive-adserver

> Sourcing limitation: Google Ad Manager documentation (support.google.com, developers.google.com, admanager.google.com) was unreachable from the research environment on 2026-09-06 after repeated attempts, and was abandoned. The canonical model above is derived from the four reachable products; claims are calibrated accordingly, and precise numeric behaviors (priority ranges, pacing defaults, cap tolerances) are intentionally not stated. Advertiser-side ad serving is evidenced through the buy-side structure of the sampled enterprise suite rather than a dedicated advertiser-side product.
